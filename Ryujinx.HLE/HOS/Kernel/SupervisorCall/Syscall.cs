...
namespace Ryujinx.HLE.HOS.Kernel.SupervisorCall
{
    class Syscall
    {
        ...
        public KernelResult SendSyncRequestWithUserBuffer(ulong messagePtr, ulong size, int handle)
        {
            ...
            byte[] messageData = process.CpuMemory.ReadBytes((long)messagePtr, (long)size);
        ...
        public KernelResult QueryMemory(ulong infoPtr, ulong position, out ulong pageInfo)
        {
            ...
            process.CpuMemory.WriteUInt64((long)infoPtr + 0x00, blkInfo.Address);
            process.CpuMemory.WriteUInt64((long)infoPtr + 0x08, blkInfo.Size);
            process.CpuMemory.WriteInt32((long)infoPtr + 0x10, (int)blkInfo.State & 0xff);
            process.CpuMemory.WriteInt32((long)infoPtr + 0x14, (int)blkInfo.Attribute);
            process.CpuMemory.WriteInt32((long)infoPtr + 0x18, (int)blkInfo.Permission);
            process.CpuMemory.WriteInt32((long)infoPtr + 0x1c, blkInfo.IpcRefCount);
            process.CpuMemory.WriteInt32((long)infoPtr + 0x20, blkInfo.DeviceRefCount);
            process.CpuMemory.WriteInt32((long)infoPtr + 0x24, 0);
        ...
        public KernelResult WaitSynchronization(ulong handlesPtr, int handlesCount, long timeout, out int handleIndex)
        {
            ...
            for (int index = 0; index < handlesCount; index++)
            {
                int handle = process.CpuMemory.ReadInt32((long)handlesPtr + index * 4);
